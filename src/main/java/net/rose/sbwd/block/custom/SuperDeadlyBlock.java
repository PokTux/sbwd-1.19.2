...
public class SuperDeadlyBlock
        extends Block {
    private final IntProvider experienceDropped;

    public SuperDeadlyBlock(Settings settings) {
        this(settings, ConstantIntProvider.create(0));
    }

    public SuperDeadlyBlock(Settings settings, IntProvider experience) {
        super(settings);
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (!entity.bypassesSteppingEffects() && entity instanceof LivingEntity && !EnchantmentHelper.hasFrostWalker((LivingEntity)entity)) {
            entity.damage(DamageSource.HOT_FLOOR, 5.0f);


        }
        if(entity instanceof LivingEntity livingEntity) {
            livingEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 200,5));
        }
